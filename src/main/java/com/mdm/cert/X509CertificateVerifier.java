...
public class X509CertificateVerifier {
	...
	public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert, 
		...
		try {
			...
			X509CrlVerifier.verifyCertificateCRLs(cert);
	 ...
	 * it. Relies on a set of root CA certificates (trust anchors) and a set of
	 * intermediate certificates (to be used as part of the chain).
	 * @param endConstraints - identity of certificate for validation
	 * @param trustedRootCerts - set of trusted root CA certificates
	 * @param intermediateCerts - set of intermediate certificates
	 * @return the certification chain (if verification is successful)
	 * @throws GeneralSecurityException - if the verification is not successful
	 ...
	 * 		chain is expired)
	 */
	public static PKIXCertPathBuilderResult verifyCertificate(X509CertSelector endConstraints, 
	    ...
	    // Configure the PKIX certificate builder algorithm parameters
	    PKIXBuilderParameters pkixParams = 
			new PKIXBuilderParameters(trustAnchors, endConstraints);
